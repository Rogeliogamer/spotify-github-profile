...
def index():
    try:
        ...
        bg_color = request.args.get('background_color', '18181b') # Fondo oscuro
        text_color = "ffffff"
        bar_color = request.args.get('bar_color', '1db954') # Verde Spotify

        if not data or not data.get('item'):
            # ESTADO: NO REPRODUCIENDO (O PAUSA)
            svg = f"""
            ...
            svg = f"""
            <svg width="350" height="100" viewBox="0 0 350 100" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                <rect x="0" y="0" width="350" height="100" rx="10" fill="#{bg_color}" stroke="rgba(255,255,255,0.1)" stroke-width="1"/>
                ...
                <rect x="105" y="75" width="{220 * (progress_pct / 100)}" height="4" rx="2" fill="#{bar_color}"/>
            </svg>
            """
